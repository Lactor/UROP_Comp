...
if KEYWORD_SET(CATALOG) then begin
    ...
    fl_ind  = where(strmatch(h_cat(*,i_line),'F*[1234567890]') eq 1,n_filt) ; find the position of the many columns
    ...
    if n_tot eq 1 then begin
        band_tot = TOTAL(fix(strmid(h_cat(tot_ind,i_line),3))) ; sum of the tot labels
        band_fl  = fix(strmid(h_cat(fl_ind,i_line),1)) ; values of the F labels
        band_ap  = fl_ind(where(band_fl eq band_tot)) ;  index where F* equals the sum of TOT*
        corr     = REFORM(cat(tot_ind,*) / cat(band_ap,*) ) ; reform cat (data) dimensions?
        corr     = TRANSPOSE(REBIN(TEMPORARY(corr),n_gal,n_filt),[1,0]) ; is this correlation values?
    endif else begin
        corr     = 1.
    endelse
    id    = fix(cat(id_ind,*),type=3) ; set ids to int
    efnu  = corr*cat(err_ind,*) ; multiplies the correlation by the error
    fnu   = corr*cat(fl_ind,*) ; multiplies the correlation by the flux
    if n_zsp eq 1 then zspec=cat(zsp_ind,*) else zspec=REPLICATE(-1,n_gal)
    rep_zsp = where(zspec lt 0,n_rep_zsp) ; indices where zspec <0
    if n_rep_zsp ge 1 then zspec(rep_zsp) = -1 ; define them as -1
    cat   = [0]

    ;...convert fnu into flambda
    eflux = 3.e8*1.e10*efnu / REBIN(lambda(0:n_filt-1) * $
                                    lambda(0:n_filt-1),n_filt,n_gal) ; error in flux
    flux  = 3.e8*1.e10*fnu / REBIN(lambda(0:n_filt-1) * $ 
                                   lambda(0:n_filt-1),n_filt,n_gal) ;  flux
    rep   = where(fnu/corr eq -99,n_rep) ;can be 0
    ...
    if FILE_TEST(CATALOG+'.zout') then begin
        ...
        if (size(h_zcat))[0] eq 1 then begin
            zcol  = where(h_zcat eq name_zphot,n_zcol) ; column of zphot
            zpcol = where(h_zcat eq 'z_spec',n_zsp2) ; column of zspec
            n_int   = N_ELEMENTS(C_INTERVAL)
            int_col = fltarr(2*n_int)
            for j=0,n_int-1 do begin
                int_col(2*j)   = where(h_zcat eq 'l'+STRING(C_INTERVAL(j),$ ; set the place of a certain value of data
                                                            f='(i2)'))
